...
function App() {
  ...
  const renderSharedHistoryLedger = () => (
    <div className={`p-6 rounded-2xl shadow-xl border w-full flex-1 flex flex-col ${theme === 'dark' ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
      <h2 className="text-sm font-black uppercase tracking-wider mb-4 flex justify-between items-center"><span>🗂️ Cloud History Feed</span><button onClick={() => setActiveSidebarTab('upload')} className="text-xs text-blue-500 hover:underline font-bold">Upload Local File</button></h2>
      ...
      {/* RESTORED OMISSION: Active Strava Link Feed block */}
      ...
      <div className="max-h-[380px] overflow-y-auto space-y-2 pr-1 scrollbar-thin scrollbar-thumb-slate-300 dark:scrollbar-thumb-slate-700">
        {historyLoading ? (
  ...
  // ---------------------------------------------------------------------------
  // NEW DASHBOARD LANDING ARCHITECTURE
  // ---------------------------------------------------------------------------
  if (!data) {
    return (
      <div className={`min-h-screen flex flex-col items-center justify-center p-4 md:p-12 relative ${theme === 'dark' ? 'bg-slate-950 text-slate-100' : 'bg-slate-50 text-slate-800'}`}>
        ...
        <div className="absolute top-4 right-4 flex items-center space-x-2 z-50">
          ...
          <button onClick={() => setTheme(theme === 'light' ? 'dark' : 'light')} className="p-2 rounded-xl border dark:border-slate-800">
  ...
  // ---------------------------------------------------------------------------
  // MAIN WORKSPACE CANVAS (DATA != NULL)
  // ---------------------------------------------------------------------------
  return (
      ...
      <div className="hidden lg:flex h-full w-full overflow-hidden flex-row">
        <div style={{ width: `${sidebarWidth}px` }} className={`flex-shrink-0 h-full overflow-y-auto p-5 shadow-sm flex flex-col space-y-6 border-r ${theme === 'dark' ? 'bg-slate-900/40 border-slate-800' : 'bg-slate-50/50 border-slate-200'}`}>
          <header className="pb-4 border-b flex justify-between items-start flex-shrink-0 dark:border-slate-800">
            <div>
              {/* FIXED: Added a dedicated white pill behind the desktop logo for pop */}
              <h1 className="text-base font-black tracking-tight flex items-center space-x-2">
                <div className="bg-white rounded p-0.5 shadow-sm flex-shrink-0">
            ...
            <div className="flex items-center space-x-1.5">
              <button onClick={() => setTheme(theme === 'light' ? 'dark' : 'light')} className="p-1.5 rounded-lg border dark:border-slate-700">
      ...
      <div className="flex lg:hidden h-screen w-full flex-col relative overflow-hidden bg-slate-50 dark:bg-slate-950 overscroll-none">
         ...
         <div className="absolute top-3 left-3 right-3 z-50 flex items-center justify-between pointer-events-none">
            <div className="p-1.5 px-2.5 bg-white/80 dark:bg-slate-900/80 text-slate-900 dark:text-slate-100 rounded-lg shadow-sm border border-slate-200/50 dark:border-slate-700/50 flex items-center space-x-2 pointer-events-auto select-none backdrop-blur-md">
               {/* FIXED: Embedded logo in a standalone white container so it doesn't get lost in dark mode */}
               <div className="bg-white rounded p-0.5 flex-shrink-0 shadow-sm">
                 <img src="/logo.png" alt="Logo" className="w-4 h-4" />
               </div>
            ...
            <div className="flex items-center space-x-1.5 pointer-events-auto">
               ...
               <button onClick={() => setMobileDrawerOpen(!mobileDrawerOpen)} title={mobileDrawerOpen ? "Hide Metrics Panel Overlay" : "Show Metrics Panel Overlay"} className="p-1.5 bg-white/90 dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 rounded-lg shadow-sm text-slate-500 dark:text-slate-400 active:bg-slate-100">
         ...
         {mobileDrawerOpen && (
            <div className={`absolute left-2 right-2 z-40 transition-all duration-300 shadow-2xl border flex flex-col p-3 rounded-xl bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 ${mobileTab === 'charts' ? 'bottom-[65px] max-h-[40vh]' : 'bottom-[65px] max-h-[50vh]'}`}>
               <div className="w-8 h-1 bg-slate-300 dark:bg-slate-700 rounded-full mx-auto mb-2.5 flex-shrink-0" />
               <div className="flex-1 overflow-y-auto min-w-0 pr-0.5 scrollbar-none">
                  ...
                  {mobileTab === 'charts' && (
                     <div className="w-full overflow-x-auto min-w-0 pb-1 scrollbar-thin scrollbar-thumb-slate-300 dark:scrollbar-thumb-slate-700 snap-x">
                        <div className="w-[1000px] h-[30vh] min-h-[220px] snap-center pr-2">
         ...
         <footer className="absolute bottom-0 left-0 right-0 h-[60px] bg-slate-950 text-white flex justify-around items-center z-50 border-t border-slate-800/80 shadow-2xl">
            <button onClick={() => { setMobileTab('summary'); setMobileDrawerOpen(true); }} className={`flex-1 h-full flex flex-col items-center justify-center space-y-0.5 text-[9px] font-black uppercase tracking-wider border-0 bg-transparent ${mobileTab === 'summary' && mobileDrawerOpen ? 'text-blue-400' : 'text-slate-500'}`}><BarChart3 className="w-4 h-4" /><span>Performance</span></button>
            <button onClick={() => { setMobileTab('charts'); setMobileDrawerOpen(true); }} className={`flex-1 h-full flex flex-col items-center justify-center space-y-0.5 text-[9px] font-black uppercase tracking-wider border-0 bg-transparent ${mobileTab === 'charts' && mobileDrawerOpen ? 'text-blue-400' : 'text-slate-500'}`}><Clock className="w-4 h-4" /><span>Timeline</span></button>
            <button onClick={() => { setMobileTab('map'); setMobileDrawerOpen(true); }} className={`flex-1 h-full flex flex-col items-center justify-center space-y-0.5 text-[9px] font-black uppercase tracking-wider border-0 bg-transparent relative ${mobileTab === 'map' && mobileDrawerOpen ? 'text-blue-400' : 'text-slate-500'}`}>
